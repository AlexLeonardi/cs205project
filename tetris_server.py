...
class Envs(object):
    ...
    def step(self, instance_id, action, render):
        ...
        #reward_jsonable = env.observation_space.to_jsonable(reward)
        #reward_jsonable = int(reward)
        done_jsonable = env.observation_space.to_jsonable(done)
        info_jsonable = env.observation_space.to_jsonable(info)
        #print(type(obs_jsonable),type(reward), type(done_jsonable), type(info_jsonable))
        #print(f'Type Obs_Jsonable: {type(obs_jsonable[0][0][0])}')
        return [obs_jsonable, int(reward), done_jsonable, info_jsonable]
