...
std::vector<UpdateDatum> PPO::update(RolloutStorage &rollouts, float decay_level)
{
    ...
    // Note: following would work in libtorch 1.5+
    // // ADDED (205)
    ...
    for (int epoch = 0; epoch < num_epoch; ++epoch)
    {
        ...
        while (!data_generator->done())
        {
            ...
            // ADDED (205): partitions mini_batch inbut to evaluate_actions
                        
            int batchsize = mini_batch.observations.size(0);
            int stride = batchsize / size ;
            
            int start = rank * stride;

            if (rank < batchsize % size){
                stride++ ;
            }

            auto evaluate_result = policy->evaluate_actions(
                mini_batch.observations.narrow(0, start, stride),
                mini_batch.hidden_states.narrow(0, start, stride),
                mini_batch.masks.narrow(0, start, stride),
                mini_batch.actions.narrow(0, start, stride));
